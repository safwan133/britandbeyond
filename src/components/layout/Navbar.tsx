...
export default function Navbar() {
 ...
 return (
    <>
      ...
      {/* Mobile drawer - Moved completely OUTSIDE the header */}
      <div
      ...
      >
        ...
        {/* Solid white drawer container */}
        <div
          className={`absolute right-0 top-0 flex h-full w-[78%] max-w-xs flex-col bg-white px-6 py-6 text-slate-900 shadow-2xl transition-transform duration-300 ease-out ${
            ...
            {NAV_ITEMS.map((item) => (
              <li key={item.href} className="border-b border-slate-200">
                <a
                  href={item.href}
                  onClick={(event) => handleNavClick(event, item.href)}
                  className="flex min-h-[56px] items-center font-display text-lg font-medium text-slate-800"
                >
                  {item.label}
                </a>
              </li>
